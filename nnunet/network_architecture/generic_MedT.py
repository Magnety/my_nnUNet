...
class AxialBlock_dynamic(nn.Module):
    ...
    def __init__(self,conv_op,norm_op,nonlin, inplanes, planes, stride=(1,1,1), downsample=None, groups=1,
        ...
        self.width_block = AxialAttention_dynamic(width, width, groups=groups, stride=stride, kernel_size=kernel_size, width=2)
        self.conv_up = conv1x1x1(width, planes * self.expansion)
        self.bn2 = norm_layer(planes * self.expansion)
...
class AxialBlock_wopos(nn.Module):
    ...
    def __init__(self,conv_op,norm_op,nonlin,  inplanes, planes, stride=(1,1,1), downsample=None, groups=1,
        ...
        self.width_block = AxialAttention_wopos(width, width, groups=groups, stride=stride, kernel_size=kernel_size,width=2)
        self.conv_up = conv1x1x1(width, planes * self.expansion)
        self.bn2 = norm_layer(planes * self.expansion)
        self.relu = nn.ReLU(inplace=True)
        self.downsample = downsample
...
class ResAxialAttentionUNet(SegmentationNetwork):
    ...
    def __init__(self, layers, input_channels,  num_classes, num_pool, num_conv_per_stage=2,
        ...
        self.layer1 = self._make_layer(self.conv_op,self.norm_op,self.nonlin,block, int(64 * s), layers[0], kernel_size=(img_size[0] ,img_size[1] // 2,img_size[2] // 2))
        self.layer2 = self._make_layer(self.conv_op,self.norm_op,self.nonlin,block, int(128 * s), layers[1], stride=(1,2,2), kernel_size=(img_size[0] ,img_size[1] // 2,img_size[2] // 2),
                                       dilate=replace_stride_with_dilation[0])
        self.layer3 = self._make_layer(self.conv_op,self.norm_op,self.nonlin,block, int(256 * s), layers[2], stride=(1,2,2), kernel_size=(img_size[0] ,img_size[1] // 4,img_size[2] // 4),
                                       dilate=replace_stride_with_dilation[1])
        self.layer4 = self._make_layer(self.conv_op,self.norm_op,self.nonlin,block, int(512 * s), layers[3], stride=(1,2,2), kernel_size=(img_size[0] ,img_size[1] // 8,img_size[2] // 8),
    ...
    def _make_layer(self,conv_op,norm_op,nonlin, block, planes, blocks, kernel_size=(8,16,32), stride=(1,1,1), dilate=False):
        norm_layer = norm_op
        downsample = None
...
class medt_net(SegmentationNetwork):
    ...
    def __init__(self, layers, input_channels,  num_classes, num_pool, num_conv_per_stage=2,
        ...
        self.relu =nn.ReLU(inplace=True)
        # self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.layer1 = self._make_layer(self.conv_op,self.norm_op,self.nonlin,block, int(128 * s), layers[0], kernel_size=(img_size[0] ,img_size[1] // 2,img_size[2] // 2))
        self.layer2 = self._make_layer(self.conv_op,self.norm_op,self.nonlin,block, int(256 * s), layers[1], stride=(1,2,2), kernel_size=(img_size[0] ,img_size[1] // 2,img_size[2] // 2),
                                       dilate=replace_stride_with_dilation[0])
        ...
        img_size_p = (img_size[0] ,img_size[1],img_size[2])
        ...
        self.layer2_p = self._make_layer(self.conv_op, self.norm_op, self.nonlin, block_2, int(256 * s), layers[1],
                                       stride=(1, 2, 2), kernel_size=(img_size_p[0], img_size_p[1] // 2, img_size_p[2] // 2),
                                       dilate=replace_stride_with_dilation[0])
        self.layer3_p = self._make_layer(self.conv_op, self.norm_op, self.nonlin, block_2, int(512 * s), layers[2],
                                       stride=(1, 2, 2), kernel_size=(img_size_p[0], img_size_p[1] // 4, img_size_p[2] // 4),
                                       dilate=replace_stride_with_dilation[1])
        self.layer4_p = self._make_layer(self.conv_op, self.norm_op, self.nonlin, block_2, int(1024 * s), layers[3],
                                       stride=(1, 2, 2), kernel_size=(img_size_p[0], img_size_p[1] // 8, img_size_p[2] // 8),
                                       dilate=replace_stride_with_dilation[2])
    ...
    def _make_layer(self,conv_op,norm_op,nonlin, block, planes, blocks, kernel_size=(128,128,128), stride=(1,1,1), dilate=False):
